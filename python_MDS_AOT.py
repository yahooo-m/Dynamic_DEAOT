...
sys.path.append('/home/deshui/pro/pro/vots2023_mds')

os.environ["CUDA_VISBLE_DEVICES"]='0'

AOT_PATH = os.path.join(os.path.dirname(__file__), 'AOT')
MIXFORMER_PATH = os.path.join(os.path.dirname(__file__), 'AOT/MixFormer')
MIXFORMER_PYTRACKING_PATH = os.path.join(os.path.dirname(__file__), 'AOT/MixFormer/external/AR/pytracking')
...
from AOT.MixFormer.lib.test.tracker.mixformer_online import MixFormerOnline
import AOT.MixFormer.lib.test.parameter.mixformer_online as vot_params
...
class DeAOTTracker(object):
    ...
    def track(self, image):
        ...
        is_valid = not torch.sum(_pred_label > 0) and conf < 0.85
...
class MSAOTTracker(object):
    def __init__(self, cfg, config):
        self.aot_tracker = DeAOTTracker(cfg, config['gpu_id'])
...
config = {
    'exp_name': 'default',
    'model': 'swinb_deaotl',
    'pretrain_model_path': 'pretrain_models/SwinB_DeAOTL_PRE_YTB_DAV.pth',
    'gpu_id': 1,
}
...
save_root = '/home/deshui/pro/pro/vots2023_mds/vis_output'
...
for mask in selection:
    mask = make_full_size(mask, (image.shape[1], image.shape[0]))
...
mask_save = make_full_size(selection[0], (image.shape[1], image.shape[0]))
mask_save = (mask_save > 0).astype(np.uint8)
out_mask = Image.fromarray(mask_save, mode="L")
colormap = imgviz.label_colormap()
out_mask.putpalette(colormap.flatten())
out_mask.save(os.path.join(save_v_dir, 'mask0.png'))
...
while True:
    ...
    for tracker in trackers:
        m_, confidence_ = tracker.track(image)
        #print(m_)
        m.append(m_)
    ...
    if True:
        ...
        image_m[:, :, 1] += 127.0 * m[0]
        image_m[:, :, 2] += 127.0 * m[0]
        contours, _ = cv2.findContours(m[0], cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        image_m = cv2.drawContours(image_m, contours, -1, (0, 255, 255), 2)
        #image_m[:, :, 0] += 127.0 * m[1]
        #image_m[:, :, 1] += 127.0 * m[1]
        #contours, _ = cv2.findContours(m[1], cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        #image_m = cv2.drawContours(image_m, contours, -1, (255, 255, 0), 2)
